...
namespace HyperMarket {
    public class HttpRequest {
        ...
        /// sending files over http. Use <see cref="HyperMarket.PostFileInfo"/> for file parameters and 
        /// <see cref="HyperMarket.PostValueParam"/> for standard string parameters
        /// </para>
        /// <para>
        /// Properties PostParams, MultipartPostParams, PostBody and PostBinaryBody are mutually exclusive
        /// use only one of them.
        ...
        public static String CombinePath(String baseUrl, params String[] folders) {
            Guard.NotNullOrEmpty(baseUrl, "baseUrl");
        ...
        internal void Validate() {
            if (String.IsNullOrWhiteSpace(Url))
                throw ErrorHelper.InvalidOperation("Url cannot be empty");
            if ((HttpVerb == HttpVerb.Get || HttpVerb == HttpVerb.Head || HttpVerb == HttpVerb.Delete) && (
                ...
                throw ErrorHelper.InvalidOperation(String.Format("Request body is not supported for {0} method", HttpVerb));
            if (new[] {
                PostParams.Return(x => x.Any(), false),
                MultipartPostParams.Return(x => x.Any(), false),
                PostBody.Return(x => x.Length > 0, false),
                ...
                throw ErrorHelper.InvalidOperation("Only one of the properties \"PostParams\", \"MultipartPostParams\", \"PostBody\" and \"PostBinaryBody\" must be populated with data. Others should be null or empty");
            if (!String.IsNullOrEmpty(UserName) && !String.IsNullOrEmpty(Password) && !String.IsNullOrWhiteSpace(BearerToken))
                throw ErrorHelper.InvalidOperation("Bearer token and Username/Password cannot be used simultaneously");
        }
